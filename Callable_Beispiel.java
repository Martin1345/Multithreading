...
public class Callable_Beispiel {// Klasse zur Demonstration der Verwendung von Callable
    public static void main(String[] args) {
        ...
        ExecutorService executor = Executors.newSingleThreadExecutor();//Erstellung eines ThreadPool, der nur einen Thread verwaltet

    
        Callable<Integer> aufgabe = () -> {// Definition der Aufgabe, die auf dem Thread ausgeführt wird
            int zahl = 21;// Zahl, mit der durch den Thread gearbeitet wird
            Thread.sleep(500); // Simulation einer Rechenzeit von 500 Millisekunden durch den Thread
            return zahl * 2;   // Durchführung der Aufgabe(verdoppeln der Zahl)und Rückgabe des Ergebnisses
        };

        try {
            // Weitergabe der Aufgabe an den Executor der diese an den Thread weitergibt
            Future<Integer> ergebnis = executor.submit(aufgabe);//Der Executor führt die Aufgabe asynchron aus und gibt ein Future-Objekt zurück
